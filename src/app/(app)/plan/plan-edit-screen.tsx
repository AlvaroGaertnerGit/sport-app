...
import { getPlan, getPlanItems, getUserRoutines } from "@/lib/domain";
...
/**
 * The actual editor body, shared by `/plan/edit` (implicitly the active
 * plan -- resolves its id first, then renders this) and
 * `/plan/[planId]/edit` (any plan, id already known) -- one implementation
 * instead of two near-identical pages, per the project's own "regla de
 * tres"/reuse-first rule. Reading + navigation this whole editor is not:
 * rename, reorder (move up/down -- no drag-and-drop, no new dependency),
 * add an existing routine, remove one, activate, archive. Nothing here is
 * a routine editor -- routines/exercises themselves stay read-only,
 * exactly as `/plan/[planId]` already treats them.
 */
export async function PlanEditScreen({ userId, planId }: { userId: string; planId: string }) {
  ...
  return (
    <div className="flex flex-1 flex-col gap-8 pt-6 pb-10">
      ...
      <RenamePlanForm planId={plan.planId} currentName={plan.name} />
            ...
            {items.map((item, index) => (
              <div key={item.planItemId} className="flex flex-col gap-2 border-b border-border py-3">
                <div className="flex items-center gap-3">
                  ...
                  <RemoveItemButton planId={plan.planId} planItemId={item.planItemId} />
                </div>
                <Link
      ...
      <div className="flex flex-col gap-3 border-t border-border pt-6">
        ...
        <ArchivePlanButton planId={plan.planId} planName={plan.name} isActive={plan.status === "active"} />
      </div>
    </div>
  );
}
