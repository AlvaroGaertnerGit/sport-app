...
// A tiny, slow, always-on drift, not a discrete gesture — this is why it
// lives here in Presence (an ambient interaction-adjacent quality) rather
// than in Personality (which only fires discrete, occasional behaviours).
// Deliberately sub-pixel-ish and slow enough that it reads as "alive," not
// as jitter. The two axes use different periods and a phase offset so the
// drift traces a lazy, irregular loop rather than a perfect circle.
...
// The antenna chases the body's own rotateY through a SECOND, softer
// spring — a "spring chasing a spring," the standard technique for
...
interface ScopePresence {
   ...
   * with the environment" rule — 0 before any real interaction has ever
   * happened (including forever, under reduced motion, since the listener
   * never attaches) and while the cursor is far away, rising only as the
   * cursor comes close to Scope. Known, accepted gap: there's no
   * `pointerleave` handling, so if the cursor exits the window entirely
   * this holds its last value rather than decaying to 0 — a small,
   * deliberately out-of-scope limitation, not an oversight.
   */
...
// rotates with it, never independently). eyeX/eyeY move only the eyes'
// focus point — lighter and faster to react than the body (see
// EYE_SPRING), with its target leading slightly on fast cursor motion
// (see EYE_LEAD_SECONDS) so it reads as anticipating rather than
// trailing, plus a small always-on jitter (see EYE_JITTER_AMPLITUDE_PX)
// so the eyes are never perfectly still. parallaxX/parallaxY give the
// display and its glow a much smaller secondary shift for a sense of
// depth. shadowX/Y/scaleX derive from the already-smoothed rotation (not
// their own spring) — a cast shadow has no physics of its own independent
// of the thing casting it. antennaRotate and interactionIntensity are two
// further derived values — see their own comments below.
function useScopePresence(
  ...
  React.useEffect(() => {
    ...
    //
    // `suspended`: the same idea for an external orchestrator that has
    // commandeered Scope — "stop gaze tracking" while suspended. Unlike
