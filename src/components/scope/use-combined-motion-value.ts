...
// subscription — a per-frame poll sidesteps the question of exactly which
// subscription mechanism is reliable for which kind of source value (a raw,
// discretely-`.set()` value like a cursor position vs. a continuously
// self-animating spring) — it just re-reads every source's `.get()` every
// frame and writes the result only when it actually changed, which is
// correct by construction regardless of how any given source got its own
// new value.
//
