...
// Scope's motion vocabulary — the one place these numbers live. "Never
// exaggerated," "light, smooth, purposeful" — the values below are those
// rules enforced as literal ceilings, not just intent.
//
...
// reference point (idle's resting baseline; happy's peak), not a value
// Framer ever animates to directly. Keep them in sync with
// SCOPE_IDLE_ANIMATE / SCOPE_HAPPY_ANIMATE below if either changes.
...
// hold — gentle breathing, almost imperceptible floating, very small body
// rotation. A fixed 2-keyframe mirror loop: deterministic, never random.
...
// The one numeric variance breathing pace has between themes — "breathing
// is a little more noticeable" in light, "breathing slightly slower" in
// dark — a ±0.2s nudge off the original 4s, read by use-scope-motion.ts.
// Everything else about idle (the keyframe shape above, the easing/
// repeatType below) is identical between themes.
...
// hold. The caller (which owns mood state) is responsible for actually
...
// "The Stillness Moment" — the one moment built from subtraction rather
// than addition: breathing itself, not a gesture, briefly stops. This
// target deliberately equals SCOPE_IDLE_ANIMATE's own resting start point
// (y: 0, rotate: 0, scale: 1) — fading TO the loop's own first frame and
// back FROM it is what makes both the entry and the exit seamless; the
// loop simply picks back up from exactly where it was already sitting,
// with no frame a visitor could point to as "when it changed." One
// transition, reused for both directions, slow enough that neither reads
// as a discrete event on its own.
...
// "under 5 seconds" for every Living Moment.
