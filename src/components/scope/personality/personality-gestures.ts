...
// replacement for it.
//
// A large, varied pool is deliberate — it's what "vary naturally, never
// loop identically" actually requires; the scheduler in
...
// (eyeOffset/eyeConverge) — this is a deliberate expansion, not a
// contradiction: body gestures move the whole creature, eye gestures move
// only the gaze, and neither one touches presence's own eyeX/eyeY
// (cursor-tracking), which compose with these via addition at the
// scope.tsx composition point.
...
interface PersonalityGestureSpec {
   ...
   * deliberately, to keep scope proportionate; a to/back spring settle
   * already gives every gesture "weight" — anticipation is a refinement on
   * top of that, not a requirement.
   */
 ...
 * springs — which `to`/`back` above use — can't sequence more than a
 * single start→end pair, so a real multi-dip blink needs a tween with an
 * explicit keyframe array instead. Still runs through the exact same
 * `activeControls`/`cancelled` cancellation machinery as every other
 * gesture — see runGesture().
 */
...
// Feedback on an earlier gesture set was "technically correct but almost
// imperceptible"; readability comes primarily through VARIETY and
// INTENTION, not bigger numbers — every value below is still a strict
// ceiling well under the mood system's own (curious/observe: 3-5° rotate,
// 3-4px y).
...
// The first-click reaction draws from this curated subset of the pool
// above, played via a shuffle bag (see shuffle-bag.ts) rather than a
// bespoke animation of its own. Every one of these gestures was already
// tuned for "subtle, under a second, physically plausible" as an idle
// behaviour — reusing them directly means the touch interaction never
// needs its own animation code for this stage at all, and a visitor who's
// already seen Scope glance or twitch his antenna idly will recognize the
// same small vocabulary, not a second, separate one. Deliberately excludes
// gestures whose read depends on genuinely idle, unprompted timing to make
// sense (widen's own "noticing something" quality fits a touch perfectly;
// squint/converge/double-blink lean more procedural/idle-specific and are
// left to the autonomous scheduler alone).
