...
// The root cause of a real deadlock, fixed by ownership: every "hold at the
// peak, then continue" step (runGesture's pool-gesture hold, runLookAt's
// hold, runTouch's three holds) used to capture its own setTimeout id into
// the SAME closure variable the idle scheduler's own scheduleNext() used
// for its "when's the next gesture" timer. Any pointermove anywhere on the
// page (onActivity, entirely unrelated to whichever hold happened to be in
// flight) called scheduleNext(), which unconditionally cleared that shared
// variable — silently orphaning whichever hold's resolve callback currently
// occupied it. That hold's `await` then never settled, its owning async
// function never returned, and (once touch() started depending on that
// function's own completion to reset its cooldown flags) the whole
// personality system could deadlock permanently from nothing more than
...
// "Look at target": an occasional glance toward a registered point of
// interest (see ScopeDockConfig.attentionTarget), layered into the exact
// same scheduling rhythm as every other gesture below rather than a
// separate timer. The reach/range geometry lives in attention-target.ts,
// kept out of this file so its own arithmetic doesn't add to this hook's
// complexity.
...
// "The Stillness Moment" — the foundational Living Moment, built from
...
// roll layered on top — true, uninterrupted inactivity this long is already
// an unusual thing for a visitor to do. Three minutes of real stillness (no
// pointer, no scroll, no keys) before it's even eligible; once played, it
// does not repeat within the same uninterrupted stretch — only genuine
// activity re-arms it (see onActivity below), so a visitor who simply walks
// away for an hour sees it exactly once, not on a repeating cadence.
...
// unsuitable for driving precise imperative timers).
...
interface ScopePersonality {
  ...
  /**
   * "The Stillness Moment" — true while Scope is holding it. Forwarded
   * into useScopeMotion(mood, isStill) at the scope.tsx composition point,
   * which owns the actual breathing loop and decides whether this produces
   * any visible effect (see that hook's own comment). A plain boolean, not
   * a new animated channel — it gates an existing one. React state,
   * deliberately, not a ref like everything else in this file: this is the
   * one value that needs to trigger a re-render (of whichever consumer
   * reads it), since it changes what scope.tsx passes to a sibling hook —
   * every other piece of state here only ever needs synchronous truth
   * inside this hook's own closures.
   */
  isStill: boolean
  /**
   ...
   * prior touch — is already mid-flight, per "ignore repeated interactions
...
// again — roughly 20-30 seconds. Reset on every successful touch, so it
// only ever fires after a genuine pause.
...
// unprompted micro-behaviour. Idle's breathing loop (SCOPE_IDLE_ANIMATE in
// ../scope-motion.ts) is Scope's accepted resting *presence*, not "a
// waiting state filled with a loop." This hook is a bounded, restrained
// extension of that same precedent — an occasional, non-repeating variation
// layered on top of the breathing baseline — not a new category of
// decorative motion.
//
...
// wherever <Scope> is rendered — no React context involved.
//
// eyeOffsetX/Y and eyeConverge are additive with presence's own eyeX/eyeY
// (composed via a useTransform sum at the scope.tsx composition point, not
// here); blinkScaleY is multiplicative with mood's own eyeScaleY (composed
// via a nested <motion.g>, not useTransform — mood's eyeScaleY isn't a
// MotionValue, so it can't be combined that way).
//
// "Movement resumes → attentive again" needs no new visual state: any
// cursor activity simply defers the idle timer (see the debounce below), so
// a gesture can never fire while the visitor is actively moving the mouse.
function useScopePersonality(
  scopeRef: React.RefObject<HTMLElement | null>,
  attentionTarget?: React.RefObject<Element | null>,
  suspended = false,
  /**
   * Any value whose *identity change* means "Scope moved on to somewhere
   * else" (in practice, the active dock id) resets the touch familiarity
   * progression back to shy. A plain optional value, exactly like
   * `attentionTarget` above and for the same reason: this hook stays fully
   * context-free.
   */
  ...
  // reruns it). Mirroring that mistake here would mean a stale target from
  // whichever dock was active when the effect last mounted. A "latest
  // value" ref, synced on every render, keeps runGesture() reading the
  // current target without needing the big effect to restart. `scopeRef`
  // doesn't need this: it's the same stable ref object for Scope's whole
  // lifetime, and only `.current` is read fresh each time, which a plain
  // ref already gives for free.
  const attentionTargetRef = React.useRef(attentionTarget)
  React.useEffect(() => {
    attentionTargetRef.current = attentionTarget
  }, [attentionTarget])

  // The "trust" touch interaction's own state, kept alongside the rest.
  // Refs, not React state, for the same reason lastGestureRef/
  // attentionTargetRef already are: touch() (below) needs synchronous,
  // immediate truth from a plain click handler, not a value that only
  ...
  // "The Stillness Moment" — see the interface's own comment on isStill
  // above for why this is React state rather than a ref, unlike everything
  // else in this file.
  const [isStill, setIsStill] = React.useState(false)

  // "Leaves wherever he was touched" — generalized as "resetSignal's own
  // identity changed." Also correctly resets on first mount (familiarity is
  ...
  // Gated only on reduced-motion, deliberately NOT on `mood`. Personality
  // is designed to be a fully independent layer, composed additively/
  // multiplicatively with Motion and Presence at the scope.tsx composition
  // point — it has no business re-deciding whether "now" is an appropriate
  // moment for mood based on a hardcoded string it doesn't own. Presence
  // already runs unconditionally the same way; Personality matches it.
  // `suspended` is an external orchestrator's "stop idle gestures" signal —
  // the exact same disarm path reduced-motion already takes below (reset
  // everything to neutral, tear down the scheduler), just temporary rather
  // than permanent for the session.
  const armed = !isReduced && !suspended

  // The Stillness Moment's isStill reset on disarm is deliberately done
  // here, during render, rather than inside the effect below — this
  // project's hooks lint disallows calling a state setter synchronously in
  // an effect body. React's own docs carve out exactly this case ("adjust
  ...
  React.useEffect(() => {
    ...
    // The Stillness Moment's own eligibility state. `lastActivityAt` tracks
    ...
    // "The Stillness Moment." Deliberately touches none of this hook's own
    // motion values — there is no gesture to play. It only sets isStill,
    // which useScopeMotion (composed at scope.tsx, see that hook's own
    // comment) reads to decide whether to pause the breathing loop. No
    // anticipation, no eye motion, no antenna flex — "no blinking, no
    // looking around" is already true for free here, since nothing in the
    // ordinary gesture pool is running concurrently (this only ever plays
    // from inside runGesture, in the same single-flight slot every other
    ...
    // even though the target may itself be in motion — geometry lives in
    // attention-target.ts.
    ...
    // The "trust" touch interaction's three reactions: the first click no
    // longer always plays the same motion. "Never presented immediately...
    // discovered gradually" — a visitor who sees the identical reaction
    // every first click would learn the trick instead of wondering about
    // the character. touch() (outside this effect) owns the cooldown/
    // familiarity bookkeeping; this function only ever plays one stage's
    // motion.
    async function runTouch(stage: TouchStage) {
      ...
      if (stage === 1) {
        ...
        // system), paired with a small, soft, restrained lift.
        activeControls = [
      ...
      // stage === 2 — the full stretch: smaller and slower than mood
      // "happy"'s hop (y -14) — this must read as settling into a stretch,
      // never jumping. The eyes hold at the deepest squint anywhere in the
      // personality system — below the pool gesture "squint" (0.6), above
      // a blink's momentary dip (0.08) — for long enough (~550ms) that it
      // reads as a deliberate, happy expression, never an involuntary
      // blink. Still the same eye pill, pushed further than anywhere else,
      // never a new shape.
    ...
    async function runGesture() {
      ...
      // The Stillness Moment gets first refusal, ahead of even the
      // attention-target glance below — the rarest, most deliberate moment
      // in the whole system should never lose a coin-flip to an ordinary
      // idle gesture on the one occasion it's actually eligible.
      // Deterministic once eligible, not an additional probability roll on
      // top: the rarity already comes from how rarely true, uninterrupted
      ...
      if (isStillnessEligible) {
        ...
        // below — deliberately NOT added to PERSONALITY_GESTURE_NAMES
        // itself: that pool is static data, this is a live-computed
        // target, and this branch leaves the existing pool's selection/
        // repeat-avoidance logic untouched.
