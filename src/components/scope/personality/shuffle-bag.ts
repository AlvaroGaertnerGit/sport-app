// A "bag randomizer": shuffle a small pool once, hand items out one at a
// time with no repeats, only reshuffle once the bag empties. This is
// deliberately NOT plain Math.random() selection — true randomness can (and
// eventually will) pick the same item twice in a row, which reads as "the
// same animation played again," not "a character with a small variety of
// behaviours." A visitor should subconsciously feel "I haven't seen him do
// that before," without ever consciously noticing why.
//
