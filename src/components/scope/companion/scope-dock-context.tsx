...
// state — these are Framer MotionValues, imperative by design) so an
// external orchestrator can temporarily animate them directly instead of
// duplicating Scope's positioning mechanism.
...
interface ScopeDockContextValue {
  ...
  /**
   * Lets a dock's own config change after registration — every other
   * dock's config is set once at mount and never touched again, so this is
   * additive, not a replacement for that convention. Bumps `configVersion`
   * so `getDockConfig`'s memoized consumer (CompanionScope) actually
   * re-renders — mutating the docks map alone wouldn't, since it's a plain
   * ref, not React state.
   */
  ...
  /**
   * True while an external orchestrator is commandeering Scope —
   ...
   * orchestrator called `beginSceneTransition()` — kept out of here on
   * purpose, this is mechanism, not policy.
   */
  isSceneTransitioning: boolean
  beginSceneTransition: () => void
  endSceneTransition: () => void
  /**
   * The mood substituted while `isSceneTransitioning` is true. Defaults to
   * "observe" — an orchestrator that needs a different mood at different
   * beats calls `setSceneMood` and resets it back to "observe" (or lets the
   * next `beginSceneTransition()` reset it) when done.
   */
  sceneMood: ScopeMood
...
function ScopeDockProvider({ children, className }: { children: React.ReactNode; className?: string }) {
  ...
  const beginSceneTransition = React.useCallback(() => {
    ...
    // orchestrator that doesn't care about mood keeps today's exact
    // behavior, and one that does starts from a known baseline rather than
    // whatever the last orchestrator left behind.
  ...
  // Unlike registerDock, this is called after mount, whenever a dock's own
  // config legitimately changes across its section's lifecycle. Every other
  // dock never calls this — a static config set once at registration
  // remains the norm.
  ...
  // Re-triggering while already acknowledging just restarts the hold window
  // rather than stacking a second reaction.
  ...
  return (
    <ScopeDockContext.Provider value={value}>
      {/* The shared "stage" every dock's position is measured relative to
          — a plain, layout-neutral position:relative box, so
          CompanionScope's absolute positioning inside it scrolls with the
          document instead of clinging to the viewport (never
          position:fixed). `className` lets a caller keep this box a
          participant in its own flex layout (see landing-page.tsx) without
          touching the positioning contract below. */}
