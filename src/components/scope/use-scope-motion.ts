...
// elements than the wrapper `animate` targets. `eyeScaleY` is deliberately
// returned separately, never folded into `animate` above — that object is
// applied to the body wrapper div in scope.tsx, while the eyes are separate
// elements deeper in the tree. Each branch below still hands back the
// *same* `transition` the body uses, so an eye's `animate={{ scaleY:
// eyeScaleY }}` in scope.tsx settles in lockstep with the body without
// needing its own transition object.
//
// `isStill` ("the Stillness Moment") is owned entirely here, not by
// Personality: breathing is this hook's own concern, and Motion is the one
// layer that actually knows whether "idle" has a loop to pause in the
// first place. Personality only ever proposes "be still right now" — a
// plain boolean, not a new animated channel — and this hook is what
...
// loop running, so the flag is silently a no-op there by construction, not
// by an extra check this hook needs to add.
function useScopeMotion(mood: ScopeMood, isStill = false) {
  const isReduced = useIsReducedMotion()
  // Replaces the reference implementation's `next-themes` `useTheme()` —
  // this app has no theme *toggle*, dark mode is pure `prefers-color-
  // scheme`, so this reads that media query directly instead.
  ...
  if (mood === "idle") {
    if (isStill) {
      ...
      // again. Glow and eyeScaleY are untouched on purpose: breathing
      // disappears and the eyes stay open, not the display dimming or
      // blinking — nothing else about idle changes.
      return {
    ...
    // Breathing pace is the one thing that varies by theme (see
    // SCOPE_IDLE_DURATION_BY_THEME) — everything else about the idle
    // transition stays exactly as authored in scope-motion.ts.
    const duration = isDark ? SCOPE_IDLE_DURATION_BY_THEME.dark : SCOPE_IDLE_DURATION_BY_THEME.light
    return {
